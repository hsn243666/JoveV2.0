...
local function run(msg, matches)
  if matches[1]:lower() == '/ژوپیتر' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /ژوپیتر ")
    return about
  end 
  if matches[1]:lower() == "ليست آمار" then
    if not is_momod(msg) then
        return "تنها براي ادمین ها !"
    end
  ...
  if matches[1]:lower() == "آمار" then
    ...
    if matches[2] == "ژوپیتر" then -- Put everything you like :)
